...
public class Statistic extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //label
        on_paint(canvas, "结余", 60,Color.BLUE, 100, 70);
        ...
        for (int i = 1; i < xTickCount; i++) {
            ...
            canvas.drawText(xLabels[position][i], x - labelWidth / 2, height - 20, xLabelPaint);
        }
        ...
        if(bills_list.size()!=0) {
            ...
            int numPoints = bills_list.size(); // 数据点的数量
            float yScale = (yStart - yEnd) / yMax; // y轴上的比例尺
            double pre_data=0;
            int count_=numPoints>7?7:numPoints;
            ...
            averageValue=averageValue/count_;
            on_paint(canvas, "平均:"+averageValue, 60,Color.BLUE, width-600, 70);
        }
